...
class PharmacyController(http.Controller):
    ...
    def book_pharmacy_detail_page(self, pharmacy_id , medicine_id, bookingNbr):
        ...
        # Create a new pharmacy.order record
        order_data = {
            'code' : bookingNbr,
            'stock_id': stock_record.id,
            'orderBookDate': fields.Datetime.now(),
            'schedule_cancel_date': fields.Datetime.now() + timedelta(minutes=30),
        }

        new_order = request.env['pharmacy.order'].sudo().create(order_data)

        # Assuming you want to mark the order as booked immediately
        new_order.make_booked()
